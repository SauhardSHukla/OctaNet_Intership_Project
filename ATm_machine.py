112# ATM Machine Simulation in Python
...
class ATMMachine:
    ...
    def check_pin(self):
        ...
        try_no = 0   # Counter for the number of attempts

        while try_no < max_try:
            entered_pin = input("Enter your PIN: ").strip()  # To remove the extra spaces

            if entered_pin == "":  #    If value is entered by user is empty
                print("PIN cannot be empty!")
            else:
                try:# To check the pin is Valid or Invalid 
    ...
    def withdraw(self, amount):
        ...
        """
        if amount > self.balance: #If bal. is too low to withdraw the cash
            print("Insufficient balance!")
        else:
            self.balance -= amount #If bal. is sufficent then withdraw the cash 
            print(f"Withdrawal successful! You withdrew {amount}.")
            self.transaction_history.append(f"Withdrawn: {amount}")#Append it to transaction histry to keep the check of all the transactions
    ...
    def deposit(self, amount):
        ...
        self.transaction_history.append(f"Deposited: {amount}") #Append it to transaction histry to keep the check of all the transactions
    ...
    def show_transaction_history(self):
        ...
        """
        if self.transaction_history: #If is hold the histry of any changes in balance 
